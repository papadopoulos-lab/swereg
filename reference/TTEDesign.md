...
a schema-2 object is an entry band row, and a 26.9.0 reader takes it for
...
- `observed_var`:
  ...
  \`"row_presence"\` asserts a trimmed skeleton, where a row exists if
  and only if the person was observed that week.
...
- `observed_var`:
  ...
  names a real logical person-week column. \`list(sentinel =
  "row_presence")\` asserts that the caller already deleted every
  unobserved person-week. A row then exists if and only if the person
  was observed that week. Person-week data MUST declare one of the two
  forms. Already-expanded trial data MAY leave this NULL. One row there
  is one trial, and not one week of observation.
