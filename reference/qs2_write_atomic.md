...
\`qs2_read()\` would halt on. \`...\` is forwarded to
\[qs2::qs_save()\].
...
What this does \*\*not\*\* promise, stated because the tempting reading
is wrong:

\* \*\*It is not durability.\*\* \`file.rename()\` is atomic with
...
protects against a killed process, not a killed machine. \* \*\*It is
not a lock.\*\* Two concurrent writers of the same \`path\` each produce
a complete file and the last rename wins. No reader sees a torn file,
but nothing here decides \*which\* writer should have won. \* \*\*It
does not always clean up after itself.\*\* The partial temp file is
...
The temporary file is created with \[tempfile()\] in the destination
