...
- dir:
  ...
  Character(1). The directory to write the `slurm/` sub-directory into.
...
A
[TTEPlan](https://papadopoulos-lab.github.io/swereg/reference/TTEPlan.md)
already holds what a job chain needs. `project_prefix` names the jobs,
`spec_version` labels them, and `expected_n_ids` with
`expected_skeleton_file_count` give the progress denominators. The
caller supplies the two facts the plan does not hold: the repository
...
`[A-Za-z0-9_.-]` replaced by one `_`. `<stage>` is the stage identity,
which is `tools::file_path_sans_ext(basename(stage))`.

Every derived name MUST match `^[A-Za-z0-9][A-Za-z0-9_.-]*$`. The
leading character carries the rule. A name that starts with `-` is a
legal file name, and a command can read it as an option.
...
- no whitespace at all:
  ...
  `dir_tteplan`, `repo`, `dir` and `stages`. These four are paths. A
  `#SBATCH` directive ends at the first whitespace character, and Slurm
  reads the rest as a separate option, so
  `#SBATCH --output=/tmp/a b/x.out` writes to `/tmp/a`.

- a whole number:

  `expected_n_ids` and `expected_skeleton_file_count`, or `NULL`, or
  `NA`.
...
identity are both valid, and it puts a live `sbatch` line into a job
script.
...
so the chain does not submit a stale script. Delete
`file.path(dir, "slurm")` first when you want the directory to hold this
...
- `--kill-on-invalid-dep=yes` on every dependant:
  ...
  A job whose `afterok` dependency can never be satisfied otherwise sits
  in the queue forever as `DependencyNeverSatisfied`.
...
- A mount check inside the job:
  ...
  first, then compares `stat -f -c %T` against the filesystem type this
  function observed at generation time.
...
- No path in the option stream:
  ...
  from `$0` in the driver. `sbatch`, `cd`, `dirname`, `head`, `stat`,
  `cat` and `awk` each take `--` as well.
...
- `dir`, `repo`, `dir_tteplan` or a stage path holds any whitespace
  character.

- `cpus` does not match `^[0-9]+$`, or `mem` does not match
  `^[0-9]+[KMGT]?$`.

- `expected_n_ids` or `expected_skeleton_file_count` is neither `NULL`,
  nor `NA`, nor one non-negative whole number.

- `cpus`, `mem` or `no_requeue` names a stage the chain does not hold.
...
which every generated job script calls.
[`vignette("tte-workflow")`](https://papadopoulos-lab.github.io/swereg/articles/tte-workflow.md)
describes the stage scripts this function chains.
