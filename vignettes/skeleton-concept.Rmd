...
This is a long-format table that defines the unit of analysis: one row per person per time unit (e.g., one row per person per week). You can think of it as the canvas. For example:
...
Once you have the skeleton, you *layer* everything else onto it. This includes:
...
The skeleton approach handles three fundamentally different types of data:
...
- Sex, birth country, genetic markers
- Baseline education level, initial diagnosis date
- Fixed characteristics from recruitment
...
swereg implements this skeleton approach through three distinct stages:
...
Each vignette builds on the previous one, showing you how to implement the skeleton approach in practice.

## Summary

Good datasets are built, not found. Most registry-based variables are *not* plug-and-play—they're constructed through a series of deliberate, transparent transformations. The skeleton-and-layers approach gives you a structure that is easy to maintain, debug, and extend. `swereg` makes this workflow easier by standardizing the painful parts. Use this approach as a blueprint: start with the skeleton, add clean layers, and keep your analysis code focused on the science—not the plumbing.