...
This vignette defines the terms used throughout the swereg target trial emulation (TTE) system. It serves as a quick reference for anyone reading or writing code that uses `TTEDesign`, `TTETrial`, or `TTEPlan`.
...
`$ipcw_pp()` combines weights (`ipw × ipcw_pp` → `analysis_weight_pp`), truncates, and drops intermediate IPCW columns in one step.
