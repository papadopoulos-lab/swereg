...
swereg uses a **per-band stratified comparator draw**. Within each enrollment band, it takes a seeded random sample of comparator individuals. The sample size is the stated ratio (e.g. 1:5) to the count of intervention individuals. The draw reads no covariate, so it is a ratio-based random sample and not covariate matching. It is a design choice for computational efficiency on a large registry dataset.
...
swereg combines the draw (for computational tractability) with IPW (to adjust for confounding by the measured baseline covariates). The IPW step re-weights the drawn sample to balance those covariates.
