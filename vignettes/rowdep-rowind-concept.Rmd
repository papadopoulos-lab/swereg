...
For transformations that need custom logic beyond first occurrence:

```{r eval=FALSE}
# Total number of hospital visits
skeleton_gd[, rowind_total_visits := sum(had_visit, na.rm = TRUE), by = .(id)]

# Years with any prescription
skeleton_gd[rx_any == TRUE, temp_year := isoyear]
skeleton_gd[, rowind_prescription_years := uniqueN(temp_year, na.rm = TRUE), by = .(id)]
skeleton_gd[, temp_year := NULL]

# Maximum income ever recorded
skeleton_gd[, rowind_max_income := max(rowdep_income, na.rm = TRUE), by = .(id)]
```
