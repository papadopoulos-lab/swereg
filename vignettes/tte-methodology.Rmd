...
|:-------|:------|:---------------------|
| Sequence of nested trials | Hernán 2008, Danaei 2013 | `TTEEnrollment$new(..., ratio=)`: Band-based enrollment assesses eligibility and treatment status weekly, then collapses `period_width` consecutive weeks into one band. Each band opens exactly one trial. Initiation in any week of a band assigns the person to that band. Time zero is the landmark, the week that closes the band, and the person must reach it under observation and event-free. See `vignette("tte-timing")`. |
...
**Note on analysis types**: swereg supports both **per-protocol** and **intention-to-treat** estimands, selected with `$s4_prepare_for_analysis(estimand = "pp" | "itt")`. Per-protocol censors follow-up at treatment switching and corrects the resulting informative censoring with IPCW (analysis weight `analysis_weight_pp[_trunc]`). Intention-to-treat keeps follow-up through switching, with no switch censoring and no IPCW, and weights on the baseline IPW alone (`ipw_trunc`). The production pipeline builds both analysis files per ETT and reports both IRRs and both risk differences side by side. As-treated analysis (time-varying IPW) is not implemented.
...
**Note on IPCW stabilization**: Danaei (2013) describes stabilized IPCW weights with a numerator conditioned on baseline covariates. That requires the same covariates in the outcome model. swereg's outcome model is covariate-free, so the numerator is a second fitted censoring model, fitted in the same strata, that carries the same follow-up-time and trial-index terms and no confounders.
