...
skeleton1_create_batch <- function(batch_ids, batch_number) {
  ...
  output_file <- file.path(OUTPUT_DIR, paste0("skeleton1_batch_", batch_number, ".qs"))
...
rm(fake_demographics, fake_annual_family, fake_inpatient_diagnoses, 
   fake_outpatient_diagnoses, fake_prescriptions, fake_cod)
...
skeleton2_clean_batch <- function(batch_number) {
  ...
  input_file <- file.path(OUTPUT_DIR, paste0("skeleton1_batch_", batch_number, ".qs"))
  ...
  output_file <- file.path(OUTPUT_DIR, paste0("skeleton2_batch_", batch_number, ".qs"))
  qs::qsave(skeleton, output_file)
...
skeleton3_analyze <- function(skeleton2_files) {
  ...
  # Final analysis dataset is ready for analysis
...
# Example: Mental health treatment patterns (among those with any mental health condition)
treatment_summary <- analysis_data[any_mental_health == TRUE, .(
  n_with_mental_health = .N,
  ...
  # Add depression treatment concordance
  depression_treatment_concordance = ifelse(sum(depression, na.rm = TRUE) > 0,
                                          mean(depression_treated[depression == TRUE], na.rm = TRUE),
                                          NA_real_)
), by = register_tag]
...
# - Large studies (500K+): BATCH_SIZE = 5000-10000
...
# qs Package Benefits:
# - 2-10x faster than RDS for large objects
...
skeleton1_create_batch_safe <- function(batch_ids, batch_number) {
  tryCatch({
    result <- skeleton1_create_batch(batch_ids, batch_number)
