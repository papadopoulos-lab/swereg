...
| 1.1     | Prevalent-user warning                                  | [`tteplan_validate_spec()`](https://papadopoulos-lab.github.io/swereg/reference/tteplan_validate_spec.md) warns when no `no_prior_intervention` exclusion covers the enrollment’s intervention level on the weekly rows of the first skeleton batch; a washout that names the right column at the wrong level does not cover it; skipped with a message when no skeleton is loaded (`global_max_isoyearweek` supplied); silence with `options(swereg.warn_prevalent_user = FALSE)` |
