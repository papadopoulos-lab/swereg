...
reading or writing code that uses `TTEDesign`, `TTETrial`, or `TTEPlan`.
...
| **person-week**      | Synonym for skeleton-level data before enrollment. The `data_level` of a `TTETrial` starts as `"person_week"`.                                                                                                                                          |
...
| **`TTETrial`**  | Trial data container (data.table + design + workflow state). Methods modify in-place via R6 reference semantics and return `invisible(self)` for `$`-chaining.                                                                                        |
