...
|:--------------------------|:--------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Sequence of nested trials | Hernán 2008, Danaei 2013  | `TTEEnrollment$new(..., ratio=)`: Band-based enrollment assesses eligibility and treatment status weekly, then collapses `period_width` consecutive weeks into one band. Each band opens exactly one trial. Initiation in any week of a band assigns the person to that band. Time zero is the landmark, the week that closes the band, and the person must reach it under observation and event-free. See [`vignette("tte-timing")`](https://papadopoulos-lab.github.io/swereg/articles/tte-timing.md).                                                                                                             |
| Cloning + censoring       | Hernán 2016               | Not used. Our per-band comparator draw is an alternative to cloning that is computationally more efficient for large registries. The draw is incidence density sampling, stratified by the `period_width`-week entry band, and it reads no other variable. It forms no matched set.                                                                                                                                                                                                                                                                                                                                  |
| Grace period              | Hernán 2016 (Section 4.4) | **Not implemented.** A true grace period (initiation allowed within X weeks of assignment without deviation) requires cloning + censoring + weighting. `period_width` provides only within-band slack for the *timing of initiation at enrollment*: treatment starting in any week of the entry band assigns the person to that band, and follow-up opens at the landmark that closes it. Deviation thereafter censors at the right edge of the first discordant run that exceeds the arm’s tolerance. See [`vignette("tte-nomenclature")`](https://papadopoulos-lab.github.io/swereg/articles/tte-nomenclature.md). |

### Weighting methods

| Method                          | Paper                     | swereg implementation                                                                                                                                                                    |
|:--------------------------------|:--------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Baseline IPW (propensity score) | Hernán 2008, Danaei 2013  | `$s2_ipw()`: Logistic regression P(A=1 \| L_baseline), stabilized by default. L_baseline is the entry-band snapshot `.tte_entry__<v>`, read at the recruiting week.                      |
...
| Weight stabilization            | Danaei 2013               | IPW: stabilized with marginal treatment probability. IPCW-PP: numerator from a second censoring model with the follow-up-time and trial-index terms and no confounders (see note below). |
...
model is covariate-free, so the numerator is a second fitted censoring
model, fitted in the same strata, that carries the same follow-up-time
and trial-index terms and no confounders.
...
| Trial as covariate                         | Caniglia 2023, Danaei 2013                                                                                                                                                                                               | Includes `trial_id` in the outcome model of `$irr()` (natural spline with 3 df for ≥5 trials, linear for 2-4) and in the IPCW models (with the default GAM, a penalised spline for ≥10 trials; otherwise linear). One trial takes no trial term.                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Risk difference and number needed to treat | None of the five papers. The NNTB and NNTH labels and the reciprocal interval come from Altman (1998, *BMJ*). Altman reports an interval that crosses zero as running through infinity; swereg reports no interval there | `$risk_difference()`: the cause-specific risk difference per band, from weighted discrete-time survival in each arm. Death censors follow-up; there is no competing-risk model. The interval is a percentile bootstrap that resamples persons, with one resample shared by both arms and the weights held fixed. The number needed to treat is `-1/rd`, labelled NNTB or NNTH by the sign of `rd`, with an interval only when the risk-difference interval excludes zero. `$s3_analyze()` runs it for both estimands. See [`vignette("tte-methods")`](https://papadopoulos-lab.github.io/swereg/articles/tte-methods.md) sections 1.2, 1.7 and 1.8. |
...
| Baseline-conditional IPCW stabilization | Would require the baseline covariates in the outcome model, which has none here. The numerator model carries the follow-up-time and trial-index terms and no confounders. |
