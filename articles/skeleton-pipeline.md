...
That is roughly 62 hours per full run. Before the three-phase migration,
editing one ICD-10 code in `code_definitions.R` meant a full 62h
rebuild, because the old pipeline ran the whole `skeleton_create()`
function end-to-end per batch and serialized the bare `data.table`.
...
The pipeline splits skeleton construction into three phases, each with
its own invalidation strategy. Per batch,
`RegistryStudy$process_skeletons()` runs the phases in order, but only
re-runs the parts that actually changed.
...
When phase 1 re-runs, phase 2 and phase 3 state are discarded and
re-applied from scratch on the fresh base.
...
`pre_2006_exclusion` reads `approach1` from `lmed_exposure`, so when you
edit `lmed_exposure`’s body, the divergence point lands at step 2 and
both step 2 and step 3 replay. That dependency is implicit in the order
...
Phase 2 runs AFTER phase 3 in the current orchestration. Phase-3 steps
CANNOT depend on phase-2 columns. If you find yourself wanting that,
push the logic into phase 3 (read raw data yourself) or restructure.
...
If `load_skeleton()` encounters a legacy pre-migration bare `data.table`
file, it wraps it in a new `Skeleton` with empty provenance fields. The
next pipeline run then sees `framework_fn_hash == NULL` and triggers a
full rebuild for that batch – the safe fallback for unknown files.
...
Primary fingerprint stability is preserved (same payload shape as before
the derived feature landed) so the release doesn’t accidentally
invalidate any existing skeletons.
...
The critical invariant: the framework, randvars, and code registry must
all be registered on `study` BEFORE `$process_skeletons()` is called.
There is no callback argument on `$process_skeletons()` – the pipeline
is declared on the study itself, not passed in per call.
...
A tiny helper at `dev/commit_pipeline_snapshot.sh` in the MHT repo wraps
the `git add` + `git commit` into a one-liner the operator runs after
the generator finishes. swereg never runs git commands itself – that
would be a side effect the user hasn’t explicitly asked for.
...
For the MHT project, the expected “edit one ICD-10 code” cost drops from
~62 hours to ~minutes. That’s the headline win.

### Summary

- **Per-batch skeleton files are `Skeleton` R6 objects** carrying
  framework hash, ordered randvars state, and the applied code registry
  fingerprint map.
