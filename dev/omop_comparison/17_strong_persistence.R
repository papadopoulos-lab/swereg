# Phase 17: easy scenario with STRONG persistence.
# Hypothesis: swereg-TTE's bias at N=50k came from censoring nearly
# all treated-baseline person-time (~99% deviation rate over 30 periods).
# With strong persistence (~95% stay-on, ~95% stay-off per period),
# both cohorts have substantial adherent person-time, and swereg should
# behave well.
...
cat("Generating N =", N, "* T =", T, "= ", N*T, "rows\n")
...
for (t in 0:(T - 1)) {
  # Strong persistence: intercept -3, prev_A coef 6
  # -> stay treated ~ plogis(3) = 95%; stay untreated ~ plogis(-3) = 5%
...
# Check adherence empirically
dt[, baseline_A := A_t[period == 0L][1L], by = id]
dt[, deviated := A_t != baseline_A]
adh <- dt[, .(any_deviation = any(deviated),
              first_dev_period = if (any(deviated)) as.numeric(min(period[deviated])) else NA_real_),
          by = id]
cat("Adherence over 30 periods:\n")
cat("  % treated at baseline:    ",
    round(100 * mean(dt[period == 0]$A_t), 1), "%\n")
cat("  % who ever deviate:       ",
    round(100 * mean(adh$any_deviation), 1), "%\n")
cat("  median first-deviation period (among deviators):",
    median(adh[!is.na(first_dev_period)]$first_dev_period), "\n")
...
cat("Oracle: ", round(coef(fit_oracle)["A_t"], 3), "\n")

# TE
te_in <- copy(dt)
te_in[, baseline_A := NULL]; te_in[, deviated := NULL]
...
# swereg
sw <- copy(dt)
sw[, baseline_A := NULL]; sw[, deviated := NULL]
...
cat("swereg analysis rows:", format(nrow(ad), big.mark = ","), "\n")
cat("(persons remaining:", uniqueN(ad$enrollment_person_trial_id), ")\n")
...
cat("\n========================================================\n")
cat("STRONG PERSISTENCE, N =", N, "(true PP =", round(TRUE_PP, 4), ")\n")
cat("========================================================\n")
...
cat("\n--- comparison with moderate persistence ---\n")
cat("Moderate persistence N=50k: TE bias -0.039, swereg bias +0.150\n")
