...
- **`TTEEnrollment$survival_curve(weight_col, save_path, title)`** —
  ...
  **time-varying** weights: pass a baseline IPW column for the ITT/IPW
  ...
  for the **PP** curve — which the old `$km()` could not do. Deaths are
  censored (no competing-risk adjustment), so the curve estimates
  event-free survival under independent censoring; label it accordingly.

### Deprecations

- **`$km()` is deprecated** and now forwards to `$survival_curve()`. The
  ...
  it. Note the return type changed from an `svykm` object to a
  `data.table` of `(treatment, tstop, hazard, surv)`.
