...
- **A function that may add columns MUST return the table, and every holder of
  that table MUST assign the return.** The rebind is a convenience and it fails
  on an expression such as `identity(sk)`, because there is no binding to write
  to. The return value is the only route the columns have there.
- The rebind reaches the IMMEDIATE caller only, so a helper that wraps such a
  call MUST reserve the slots itself or return the table.
- Two slot figures, both in `R/dt_alloc.R`. A SKELETON gets
  `.DT_ALLOC_SPARE_SLOTS`, which is 4096. Every other data.table gets
  `.DT_ALLOC_NESTED_SLOTS`, which is 1024. A plan holds many small tables and
  each free slot costs 16 bytes, used or not. Do not read
  `options(datatable.alloccol)`.
- **State the applier's guarantee in exactly these three parts.** Every
  column the entry declares is present. The four structural columns satisfy
  `identical()`, attributes included. No pre-existing column is dropped, and none is replaced
  by a new vector. **Do not write "in-place mutation is enforced".** A
  registered function holds a reference to the skeleton and can write into an
  existing vector, and nothing sees that.
- `.validate_columns_kept()` (`R/validation_helpers.R`) decides replacement by
  COLUMN address, never by header address. `setalloccol()` moves the header
  and keeps every column vector. An entry MAY replace a column it declares,
  because registration makes ownership unique.
- **The four structural columns are compared by VALUE, once per entry.** A
  `:=` that keeps the column type writes into the existing vector, so neither
  the row count nor the address moves. One snapshot is cheap relative to a
  code entry.
- **`register_codes()` and `register_derived_codes()` check two things.**
  Every group an entry names MUST be in `group_names`. No column an entry
  generates MAY be generated by another entry.
- **A group with no rawbatch row still calls `fn`, with the zero-row table.**
  `save_rawbatch()` writes a file per group per batch, and
  `payload_for_batch()` yields a zero-row slice when no person matches.
  `.entry_columns(reg)` MUST be present once an entry has run, because
  `$apply_code_entry()` records the entry straight afterwards.
- **A failed entry leaves its partial writes on the in-memory skeleton.**
  There is nothing to roll back to. `$process_skeletons()` never writes that
  batch and halts the run, so no partial batch reaches disk.
...
- The registry path is pinned by `tests/testthat/test-dt-alloc-registry.R`.
