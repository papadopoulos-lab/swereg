...
#
# We pin both:
#   1. Source-level: tteplan_sensitivity_sheet.R no longer contains the old
#      strings.
...
test_that("source: tteplan_sensitivity_sheet.R uses Intervention / (int) labels (not Exposure / (exp))", {
  src_path <- testthat::test_path(
    "..",
    "..",
    "R",
    "tteplan_sensitivity_sheet.R"
  )
  skip_if_not(
    file.exists(src_path),
    "R/tteplan_sensitivity_sheet.R not found (installed pkg?)"
  )
  src <- readLines(src_path, warn = FALSE)

  # New labels must appear (sensitivity sheet header + per-arm cols).
  expect_true(
    any(grepl('"Intervention"', src, fixed = TRUE)),
    info = '"Intervention" header missing from tteplan_sensitivity_sheet.R'
  )
  expect_true(
    any(grepl('"Events (int)"', src, fixed = TRUE)),
    info = '"Events (int)" column header missing from tteplan_sensitivity_sheet.R'
  )
