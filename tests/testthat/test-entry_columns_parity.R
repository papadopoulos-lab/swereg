...
# columns.
#
# Coverage: add_diagnoses, add_cancer_without_morphology, add_operations, add_rx (source =
...
# ---------------------------------------------------------------------------
# add_cancer_without_morphology (cancer registry)
# ---------------------------------------------------------------------------

test_that(".entry_columns matches add_cancer_without_morphology", {
  skeleton <- .parity_skeleton()
  batch_data <- .parity_batch_data_diagnoses()
  reg <- list(
...
test_that(".entry_columns matches add_quality_registry", {
  ...
  if (nrow(skeleton) == 0L) {
    skip("no overlapping ids in parity skeleton")
  }
