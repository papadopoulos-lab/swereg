...
# ---------------------------------------------------------------------------
# applied_registry is normalized to registration order after a sync
# ---------------------------------------------------------------------------
#
# This is the 2026-09-21 fix. $sync_with_registry() re-applies a changed entry
# at the END, so an edit to any entry but the last left the stored map in
# application order while the study stayed in registration order. Nothing
# about the data changed, yet the end gate rejected a complete 2194-batch
# rebuild AND the per-batch fast path in .meta_matches_pipeline() would have
# missed on every batch of every later run: load the whole skeleton, find
# nothing to do, save it again, forever.

.sk_with_entries <- function(fps) {
  sk <- Skeleton$new(data = .sk_dt(), batch_number = 1L)
  sk$framework_fn_hash <- "fw"
  for (k in seq_along(fps)) {
    entry <- .fake_entry(
      codes = stats::setNames(list("X"), paste0("c", k)),
      groups = list(grp_a = "grp_a")
    )
    sk$apply_code_entry(
      entry, .fake_batch_data(), id_col = "id", fingerprint = fps[[k]]
    )
  }
  return(sk)
}

test_that("a re-applied entry is put back into registration order", {
  fps <- c("fp_a", "fp_b", "fp_c")
  sk <- .sk_with_entries(fps)
  expect_identical(names(sk$applied_registry), fps)

  # Reproduce the incident: the middle entry changes, so it is dropped and
  # re-applied at the end.
  sk$drop_code_entry("fp_b")
  sk$apply_code_entry(
    .fake_entry(codes = list(c2 = "X"), groups = list(grp_a = "grp_a")),
    .fake_batch_data(), id_col = "id", fingerprint = "fp_b"
  )
  expect_identical(names(sk$applied_registry), c("fp_a", "fp_c", "fp_b"))

  swereg:::.reorder_applied_registry(sk, fps)
  expect_identical(names(sk$applied_registry), fps)
})

test_that("the normalized order makes the meta fast path match again", {
  fps <- c("fp_a", "fp_b", "fp_c")
  sk <- .sk_with_entries(fps)
  sk$drop_code_entry("fp_b")
  sk$apply_code_entry(
  ...
  fast <- function(sk) {
    swereg:::.meta_matches_pipeline(
      meta = list(
        schema_version = swereg:::.REGISTRY_STUDY_SCHEMA_VERSION,
      ...
      framework_hash = sk$framework_fn_hash,
      trim_hash = sk$trim_fn_hash,
      phase_order = sk$phase_order,
      randvars_hashes = character(0),
      current_fps = fps
    )
  }
  # Before the sync the fast path misses, which is the wasted full load.
  expect_false(fast(sk))

  swereg:::.reorder_applied_registry(sk, fps)
  expect_true(fast(sk))
})

test_that("an entry not in the registry is kept, at the end, not dropped", {
  sk <- .sk_with_entries(c("fp_a", "fp_b"))
  swereg:::.reorder_applied_registry(sk, c("fp_b"))
  expect_identical(names(sk$applied_registry), c("fp_b", "fp_a"))
})

test_that("the reorder reaches a skeleton carrying OLD method bodies", {
  # The release-blocking case. An R6 object serializes its methods, so every
  # skeleton already on disk runs the sync body of the swereg that wrote it.
  # A fix inside that method would never reach them. This fixture has no
  # $sync_with_registry at all, which is the strongest form of "its methods
  # are not this version's": the free function must still normalize it.
  # An environment, because that is what an R6 object is: the reorder relies
  # on reference semantics, and a plain list would silently not be mutated.
  legacy <- new.env()
  legacy$applied_registry <- list(
    fp_c = list(x = 3), fp_a = list(x = 1), fp_b = list(x = 2)
  )
  swereg:::.reorder_applied_registry(legacy, c("fp_a", "fp_b", "fp_c"))
  expect_identical(names(legacy$applied_registry), c("fp_a", "fp_b", "fp_c"))
  # The descriptors travel with their keys, they are not re-indexed by position.
  expect_identical(legacy$applied_registry$fp_a$x, 1)
  expect_identical(legacy$applied_registry$fp_c$x, 3)
})
