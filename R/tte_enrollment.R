...
# collapse, ipw, prepare_for_analysis, truncate, etc.
# Enrollment (enroll), outcome prep (prepare_outcome), IPCW (ipcw_pp), and
...
TTEEnrollment <- R6::R6Class("TTEEnrollment",
  public = list(
    ...
    initialize = function(data, design, data_level = "trial",
      ...
      if (!is.null(ratio)) {
        private$enroll(ratio = ratio, seed = seed, extra_cols = extra_cols)
      }
    },
    ...
    collapse = function(
      ...
      if (self$data_level != "trial") {
        stop(
          ...
          "Hint: Pass ratio to tte_enrollment() to convert person_week data to trial level."
        )
      }
    ...
    #' @description Prepare outcome data and calculate IPCW-PP in one step.
    ...
    prepare_for_analysis = function(
      ...
      private$prepare_outcome(outcome = outcome, follow_up = follow_up)
      if (is.null(censoring_var)) censoring_var <- "censor_this_period"
      private$ipcw_pp(
    ...
    rates = function(weight_col) {
      ...
      if (!"event" %in% names(data)) {
        stop("'event' column not found. Run $prepare_for_analysis() first.")
      }
    ...
    irr = function(weight_col) {
      ...
      keep_cols <- unique(c(
        design$person_id_var, design$exposure_var, design$tstop_var,
        weight_col, "event", "person_weeks"
      ))
    ...
    km = function(ipw_col, save_path = NULL, title = NULL) {
      ...
      keep_cols <- unique(c(
        design$id_var, design$person_id_var, design$exposure_var,
        design$tstop_var, ipw_col, "event"
      ))
  ...
  private = list(
    ...
    enroll = function(ratio = 2, seed = NULL, extra_cols = NULL) {
      if (self$data_level != "person_week") {
        stop(
          ...
          "Current data_level: '", self$data_level, "'\n",
          "Hint: Pass ratio to tte_enrollment() with person_id_var in design."
        )
      }
    ...
    ipcw_pp = function(
      ...
      if (!censoring_var %in% names(self$data)) {
        stop("censoring_var '", censoring_var, "' not found. Run $prepare_for_analysis() first.")
      }
  ...
  active = list(
    ...
    enrollment_stage = function() {
      if (self$data_level == "person_week") return("pre_enrollment")
      if ("prepare_outcome" %in% self$steps_completed) return("analysis_ready")
      "enrolled"
    }
  )
)
...
#' @param ratio Numeric or NULL. If provided, automatically enrolls
#'   participants (sampling comparison group and creating trial panels).
#'   Only valid for person_week data.
#' @param seed Integer or NULL. Random seed for enrollment reproducibility.
#' @param extra_cols Character vector or NULL. Extra columns to include in
#'   trial panels during enrollment.
#'
...
tte_enrollment <- function(data, design, data_level = NULL,
                           ratio = NULL, seed = NULL, extra_cols = NULL) {
