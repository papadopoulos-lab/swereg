...
# =============================================================================
# skeleton_save: Save skeleton batch to disk
...
#'
#' Saves a skeleton data.table as a single \code{skeleton_{BBB}.qs2} file.
#'
...
#'
#' @return The file path created (length-1 character).
#'
#' @examples
#' \dontrun{
#' path <- skeleton_save(
...
#' @export
skeleton_save <- function(dt, batch_number, output_dir) {
  data.table::setattr(dt, "created_at", Sys.time())

  n_threads <- max(1L, floor(parallel::detectCores() / 2))
  fpath <- file.path(output_dir, sprintf("skeleton_%03d.qs2", batch_number))
  qs2::qs_save(dt, fpath, nthreads = n_threads)
