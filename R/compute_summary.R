...
.bucket_section_lines <- function(
  ...
  for (i in seq_along(buckets)) {
    pfx <- names(buckets)[i]
    out <- c(out, sprintf("    %-*s  %*s", pad_pfx, pfx, pad_num, num_strs[i]))
  }
  out <- c(out, "")
    ...
    if (show_counts) {
      ...
      for (i in ord) {
        out <- c(
          out,
          sprintf(
            "    %-58s n_persons=%s  weeks=%s  years=%s",
            b$column_name[i],
...
.write_status_txt <- function(summary, path) {
  ...
  lines <- c(
    lines,
    sprintf("Built:           %s", format(meta$built_at, "%Y-%m-%dT%H:%M:%SZ"))
  )
  ...
  lines <- c(
    ...
    sprintf(
      "Skeletons:       %d / %d (%s)",
      meta$n_batches_present,
      meta$n_batches_expected,
      status_word
    )
  )
  if (!meta$is_complete) {
  ...
  if (length(meta$missing_counts_batches) > 0L) {
    lines <- c(
      lines,
      sprintf(
        "Note: %d batches lack per-column counts and were skipped in totals.",
  ...
  lines <- c(
    lines,
    sprintf(
      "Time period with WEEKLY data: %s to %s",
  ...
  # --- Annual data section ---
  ay_min <- rw$annual_period_min
  ay_max <- rw$annual_period_max
  lines <- c(
    lines,
    sprintf(
      "Time period with ANNUAL data: %s to %s",
  ...
  lines <- c(
    lines,
    sprintf(
      "[ok] Variables with n_persons_with >= 10:  %s of %s total",
      comma(okn),
      comma(nrow(cols))
    )
  )
  ...
  lines <- c(
    lines,
    .bucket_section_lines(
      sprintf(
        "[!] Variables that NEVER matched (n_persons_with == 0): %s",
        comma(nrow(never))
      ),
      never,
      show_counts = FALSE,
  ...
  lines <- c(
    ...
    .bucket_section_lines(
      sprintf(
        "[!] Variables matched but VERY RARE (n_persons_with 1-9): %s",
        comma(nrow(rare))
      ),
      rare,
      show_counts = TRUE,
  ...
  lines <- c(
    lines,
    sprintf("  qs2::qs_read(\"<data_skeleton_dir>/summary.qs2\")")
  )
...
.write_summary_tsv <- function(summary, path, suppress_below = 5L) {
  ...
  header <- c(
    ...
    sprintf(
      "# n_batches\t%d / %d (complete=%s)",
      meta$n_batches_present,
      meta$n_batches_expected,
      tolower(as.character(meta$is_complete))
    ),
    sprintf(
      "# weekly_period\t%s to %s",
      rw$weekly_period_min %||% "NA",
      rw$weekly_period_max %||% "NA"
    ),
    sprintf(
      "# annual_period\t%s to %s",
      if (is.na(rw$annual_period_min)) {
        "NA"
      } else {
        as.character(rw$annual_period_min)
      },
      if (is.na(rw$annual_period_max)) {
        "NA"
      } else {
        as.character(rw$annual_period_max)
      }
    ),
