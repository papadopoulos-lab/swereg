...
- **Phase 2 (the one runner): DONE — signed off, adversarially, over
  ...
  **enrollment-loop** migration are in. codex
  ...
  - R7 (1): exact `[[` extraction everywhere (no `$` partial-matching
    steering loads).
  - R8: **DONE — YES.** Phase 3 remains:
    `save_rawbatch`/`.batch_stream`, the remaining shape-A sites,
    `process_skeletons` (with the snapshot trick), deleting
    `parallel_pool`/`callr`/the 9 workers, and the “one dispatcher only”
    test.
...
- **Next: Phase 2** (`R/batch.R`, the one runner). Not started. The
  sign-off’s carry-forward: *make the generic runner the only real
  process boundary, then test failures through that production boundary*
  – helper tests and dependency demonstrations must not substitute for
  proving the actual caller -\> dispatcher -\> worker -\> cleanup -\>
  completion path is wired right. Three of the six rounds’ findings were
  my own tests passing for the wrong reason; that lesson is the reason
  to insist on production-boundary tests in Phase 2.
...
but not in the four other paths that divide it;
...
plan** — no s3 rerun needed. - Next: Phase 2 (`R/batch.R`, the one
runner). Not started.
...
`.pp_log_tail()` was also lifted out of
[`parallel_pool()`](https://papadopoulos-lab.github.io/swereg/reference/parallel_pool.md)’s
closure so it can be unit-tested at all, and hardened against non-text
worker output: an embedded NUL made
...
[`parallel_pool()`](https://papadopoulos-lab.github.io/swereg/reference/parallel_pool.md)’s
docstring no longer overstates its transport (“avoids serialization
entirely” → qs2-vs-RDS file transport).
...
script *in the consumer’s dev tree*, and `.batch_stream`’s dev branch
`load_all`s only the consumer. Once `batchit` is the runner, the worker
script comes from the **installed runner** while `dev_path` is the
