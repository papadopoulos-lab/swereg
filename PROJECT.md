...
- **Phase 5' DONE** — swereg 26.8.1 (632bd90) removed s1's `resume`, s2's
  `.resume_fresh()` + 24 h window, and s3's `force=` / skip-if-cached; MHT repo
  61d5186 dropped the now-dead arguments from the 002/003/006 stage scripts.
...
- **Gate 0 — full MHT production baseline (unchanged).** Before *any* Phase 5'/6' code, run
  the full MHT production pipeline on the current stack; record runtime / R / package
  ...
  `bash bin/generic.sh && bash bin/tte.sh 002-ozel-psychosis` (~1.5–2 days). (This is the same production rerun the
